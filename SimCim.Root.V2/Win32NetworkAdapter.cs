...
namespace SimCim.Root.V2
{
    public class Win32NetworkAdapter : CIMNetworkAdapter
    {
        ...
        public Win32NetworkAdapterConfiguration ResolveWin32NetworkAdapterSettingSetting()
        {
            var instances = InfrastuctureObjectScope.CimSession.EnumerateAssociatedInstances("root/cimv2", InnerCimInstance, "Win32_NetworkAdapterSetting", "Win32_NetworkAdapterConfiguration", "Element", "Setting");
            return instances.Select(i => (Win32NetworkAdapterConfiguration)InfrastuctureObjectScope.Mapper.Create(i)).SingleOrDefault();
        }

        public Win32NetworkProtocol ResolveWin32ProtocolBindingAntecedent()
        {
            var instances = InfrastuctureObjectScope.CimSession.EnumerateAssociatedInstances("root/cimv2", InnerCimInstance, "Win32_ProtocolBinding", "Win32_NetworkProtocol", "Device", "Antecedent");
            return instances.Select(i => (Win32NetworkProtocol)InfrastuctureObjectScope.Mapper.Create(i)).SingleOrDefault();
        }
    }
}