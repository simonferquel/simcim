...
namespace SimCim.Root.Virtualization.V2
{
    public abstract class EventConsumer : IndicationRelated
    {
        ...
        public EventFilter ResolveFilterToConsumerBindingFilter()
        {
            var instances = InfrastuctureObjectScope.CimSession.EnumerateAssociatedInstances("root/virtualization/v2", InnerCimInstance, "__FilterToConsumerBinding", "__EventFilter", "Consumer", "Filter");
            return instances.Select(i => (EventFilter)InfrastuctureObjectScope.Mapper.Create(i)).SingleOrDefault();
        }
    }
}