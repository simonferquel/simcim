...
namespace SimCim.Root.Virtualization.V2
{
    public class EventFilter : IndicationRelated
    {
        ...
        public EventConsumer ResolveFilterToConsumerBindingConsumer()
        {
            var instances = InfrastuctureObjectScope.CimSession.EnumerateAssociatedInstances("root/virtualization/v2", InnerCimInstance, "__FilterToConsumerBinding", "__EventConsumer", "Filter", "Consumer");
            return instances.Select(i => (EventConsumer)InfrastuctureObjectScope.Mapper.Create(i)).SingleOrDefault();
        }
    }
}