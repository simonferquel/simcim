...
namespace SimCim.Root.Virtualization.V2
{
    public abstract class CIMResourceAllocationSettingData : CIMSettingData
    {
        ...
        {
            var instances = InfrastuctureObjectScope.CimSession.EnumerateAssociatedInstances("root/virtualization/v2", InnerCimInstance, "Msvm_EthernetPortFailoverSettingDataComponent", "Msvm_FailoverNetworkAdapterSettingData", "GroupComponent", "PartComponent");
            return instances.Select(i => (MsvmFailoverNetworkAdapterSettingData)InfrastuctureObjectScope.Mapper.Create(i));
        }

        public CIMResourcePool ResolveCIMResourceAllocationFromPoolAntecedent()
        {
            var instances = InfrastuctureObjectScope.CimSession.EnumerateAssociatedInstances("root/virtualization/v2", InnerCimInstance, "CIM_ResourceAllocationFromPool", "CIM_ResourcePool", "Dependent", "Antecedent");
            return instances.Select(i => (CIMResourcePool)InfrastuctureObjectScope.Mapper.Create(i)).SingleOrDefault();
        }

        public CIMResourcePool ResolveMsvmResourceAllocationFromPoolAntecedent()
        {
            var instances = InfrastuctureObjectScope.CimSession.EnumerateAssociatedInstances("root/virtualization/v2", InnerCimInstance, "Msvm_ResourceAllocationFromPool", "CIM_ResourcePool", "Dependent", "Antecedent");
            return instances.Select(i => (CIMResourcePool)InfrastuctureObjectScope.Mapper.Create(i)).SingleOrDefault();
        }
