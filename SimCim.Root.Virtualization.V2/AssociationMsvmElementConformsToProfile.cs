...
namespace SimCim.Root.Virtualization.V2
{
    public struct MsvmElementConformsToProfileAssociation
    {
        ...
        public IEnumerable<MsvmComputerSystem> ManagedElement(CimInstance inConformantStandard)
        {
            var scope = _scope;
            var instances = _resolver.ResolveTarget(scope, inConformantStandard);
            return instances.Select(i => (MsvmComputerSystem)scope.Mapper.Create(scope, i));
        }

        public IEnumerable<CimInstance> ConformantStandard(MsvmComputerSystem inManagedElement)
        {
            var scope = _scope;
