using Microsoft.Management.Infrastructure;
using Microsoft.Management.Infrastructure.Options;
using SimCim.Core;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Reactive.Linq;

namespace SimCim.Root.Virtualization.V2
{
    public struct MsvmMetricInstanceAssociation
    {
        private static AssociationResolver _resolver = new AssociationResolver("Msvm_MetricInstance", "CIM_BaseMetricDefinition", "CIM_BaseMetricValue", "Antecedent", "Dependent");
        private IInfrastructureObjectScope _scope;
        public MsvmMetricInstanceAssociation(IInfrastructureObjectScope scope)
        {
            _scope = scope;
        }

        public IEnumerable<CIMBaseMetricValue> Dependent(CIMBaseMetricDefinition inAntecedent)
        {
            var scope = _scope;
            var instances = _resolver.ResolveTarget(scope, inAntecedent.AsCimInstance());
            return instances.Select(i => (CIMBaseMetricValue)scope.Mapper.Create(scope, i));
        }

        public IEnumerable<CIMBaseMetricDefinition> Antecedent(CIMBaseMetricValue inDependent)
        {
            var scope = _scope;
            var instances = _resolver.ResolveSource(scope, inDependent.AsCimInstance());
            return instances.Select(i => (CIMBaseMetricDefinition)scope.Mapper.Create(scope, i));
        }

        public IObservable<CIMBaseMetricValue> DependentAsync(CIMBaseMetricDefinition inAntecedent, CimOperationOptions options = null)
        {
            var scope = _scope;
            var instances = _resolver.ResolveTargetAsync(scope, inAntecedent.AsCimInstance(), options);
            return instances.Select(i => (CIMBaseMetricValue)scope.Mapper.Create(scope, i));
        }

        public IObservable<CIMBaseMetricDefinition> AntecedentAsync(CIMBaseMetricValue inDependent, CimOperationOptions options = null)
        {
            var scope = _scope;
            var instances = _resolver.ResolveSourceAsync(scope, inDependent.AsCimInstance(), options);
            return instances.Select(i => (CIMBaseMetricDefinition)scope.Mapper.Create(scope, i));
        }
    }
}