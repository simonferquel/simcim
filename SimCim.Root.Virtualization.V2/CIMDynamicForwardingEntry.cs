...
namespace SimCim.Root.Virtualization.V2
{
    public abstract class CIMDynamicForwardingEntry : CIMLogicalElement
    {
        ...
        public CIMSwitchPort ResolveCIMSwitchPortDynamicForwardingAntecedent()
        {
            var instances = InfrastuctureObjectScope.CimSession.EnumerateAssociatedInstances("root/virtualization/v2", InnerCimInstance, "CIM_SwitchPortDynamicForwarding", "CIM_SwitchPort", "Dependent", "Antecedent");
            return instances.Select(i => (CIMSwitchPort)InfrastuctureObjectScope.Mapper.Create(i)).SingleOrDefault();
        }

        public CIMTransparentBridgingService ResolveCIMTransparentBridgingDynamicForwardingAntecedent()
        {
            var instances = InfrastuctureObjectScope.CimSession.EnumerateAssociatedInstances("root/virtualization/v2", InnerCimInstance, "CIM_TransparentBridgingDynamicForwarding", "CIM_TransparentBridgingService", "Dependent", "Antecedent");
            return instances.Select(i => (CIMTransparentBridgingService)InfrastuctureObjectScope.Mapper.Create(i)).SingleOrDefault();
        }
    }
}